...
# ---------------------------------------------------------------------------
ALL_APPS="caddy walls litellm openwebui infisical registry registry-public dev-db honcho"
...
pvcs_for_app() {
  case "$1" in
    ...
    registry-public) echo "registry-public-data" ;;
    dev-db) echo "dev-db-postgres-data" ;;
    honcho)   echo "honcho-postgres-data" ;;
    *)        return 1 ;;
  esac
}
